...
function employeeForm(emp={}){ return `<form id="employeeForm" class="form-grid"><input type="hidden" name="id" value="${esc(emp.id||``)}"><label>الاسم الكامل<input name="fullName" required value="${esc(emp.fullName||``)}"></label><label>البريد<input name="email" type="email" value="${esc(emp.email||``)}"></label><label>اسم المستخدم<input name="username" value="${esc(emp.username||``)}"></label><label>الدور<input name="role" value="${esc(emp.role||`sales_rep`)}"></label><label>تاريخ البداية<input name="startDate" type="date" value="${esc(emp.startDate||``)}"></label><label>الراتب الشهري<input name="normalMonthlySalary" type="number" step="0.001" value="${number(emp.normalMonthlySalary)}"></label><label>النقد<input name="cashBalance" type="number" step="0.001" value="${number(emp.cashBalance)}"></label><label>CliQ<input name="cliqBalance" type="number" step="0.001" value="${number(emp.cliqBalance)}"></label><label>رصيد السلف<input name="advancesBalance" type="number" step="0.001" value="${number(emp.advancesBalance)}"></label><label>رصيد راتب سابق<input name="salaryBalance" type="number" step="0.001" value="${number(emp.salaryBalance)}"></label><label>الحالة<select name="status"><option value="active" ${emp.status!==`inactive`?`selected`:``}>فعال</option><option value="inactive" ${emp.status===`inactive`?`selected`:``}>غير فعال</option></select></label><p class="hint wide">سيارة المندوب تُربط تلقائياً من إدارة المستخدمين ولا تُحدّد يدوياً.</p><label class="wide">ملاحظات<textarea name="notes">${esc(emp.notes||``)}</textarea></label></form>`; }
function bindEmployees(root, users, data, user){
  ...
  async function showEmp(emp={}){ const {modal}=await import('./utils.js'); modal(emp.id?`تعديل موظف`:`إضافة موظف`,employeeForm(emp),[{label:`حفظ`,className:`primary`,handler:async wrap=>{const f=$(`#employeeForm`,wrap); if(!f.reportValidity())return; const d=getFormData(f); const payload={...d,normalMonthlySalary:number(d.normalMonthlySalary),cashBalance:number(d.cashBalance),cliqBalance:number(d.cliqBalance),advancesBalance:number(d.advancesBalance),salaryBalance:number(d.salaryBalance)}; if(d.id) await erp.update(`users`,d.id,payload); else return toast(`إضافة الموظفين تتم من إدارة النظام فقط`,`err`); wrap.remove(); toast(`تم حفظ الموظف`); location.reload();}}]); }
}
