...
function bindAdmin(root, users){
  ...
  async function filterLogs(e){
    ...
    const logs=(await erp.list(`systemLogs`,{includeDeleted:true})).filter(l=>(!q||JSON.stringify(l).toLowerCase().includes(q))&&(!module||l.module===module)&&(!action||l.actionType===action));
